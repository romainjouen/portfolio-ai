...
load_dotenv()
model = os.getenv('LLM_MODEL', 'qwen')  # Default to qwen if not specified

# Use different models for different agents
ROUTER_MODEL = os.getenv('ROUTER_MODEL', 'gemini-pro')
AGENT_MODEL = os.getenv('AGENT_MODEL', 'gpt-3.5')
...
sql_router_agent = Agent(
    ...
    model=MODELS[ROUTER_MODEL].model_name
)
rss_feed_agent = Agent(
    ...
    model=MODELS[AGENT_MODEL].model_name
)
user_agent = Agent(
    name="User Agent",
